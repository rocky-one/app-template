...
1. https配置，先去腾讯云申请https下载证书，拷贝证书文件到nginx目录下
2. 配置nginx.conf文件，支持https ssl:443
...
停止
docker stop 'name'（通过上面查看name）

创建软连接
ln -s /usr/local/node-v16.15.1-linux-x64/bin/pm2 /usr/bin/pm2

nginx启动  monkey-grid文档代码
1、启动nginx：./nginx 
2、关闭nginx：./nginx  -s  stop
3、重启nginx：./nginx -s reload

http://www.monkey-grid.com/
...
2. db.createUser({user:"ai",pwd:"smileAa.12",roles:[{role:"userAdminAnyDatabase",db:"admin"},"readWriteAnyDatabase"]})


// mongodb 创建用户 并分配权限
db.createUse({user:"ai",pwd:"smileAa.12",roles:[{role:"root",db:"admin"}]})

web-ai-node
1. 启动node服务
2. 配置nginx转发到本地node服务器

web-ai
1. web页面使用nginx转发到node server.js开始的端口，因为有服务端渲染所有web页面需要启动node server。
  - 先打包 npm run build
  - 启动node ssr服务
2. 接口使用nginx端口转发到本地的node服务器，node使用pm2启动
